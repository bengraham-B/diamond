...
const diamondUserAuth = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        console.log({ "Verify User": query.rows });
        ...
        if (((_a = query.rowCount) !== null && _a !== void 0 ? _a : 0) > 0) {
            ...
            console.log("Found User AccountID [account table]", founudUserQuery.rows);
            return res.status(200).json({
                authenticatedDiamondUser: founudUserQuery.rows[0],
                msg: `User Exists Already`
            });
        }
        // Y DIAMOND_USER does not exist so one will be created
        else {
            console.log({ f: "USER DOES NOT EXIST" });
            ...
            try {
                ...
                console.log("diamondUser.controller", error);
                return res.status(500).json({ error: error, msg: `Cannot Add diamond_user to DB` });
            }
        }
    }
    catch (error) {
        console.log("diamondUser.controller", error);
        return res.status(500).json({ error: error, msg: `Cannot Verify diamond_user in DB` });
    }
});
